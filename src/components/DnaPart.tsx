...
export function DnaPart({ label, part }: { label: string; part: DnaFacePart }) {
  ...
  const updateBlend = useCallback(
    (index: number, value: number) => {
      ...
      const newBlends = newValues.map((v, i) => ({
        headId: blend[i].headId,
        value: v,
      }));
      updateCharacter((d) => {
        d.dna.blends[part] = newBlends;
      });
    },
  ...
  return (
    <Fieldset legend={label}>
      <DnaBlend headId={blend[0].headId} value={blend[0].value} onChangeNumber={(v) => updateHeadId(0, v)} onChangeSlider={(v) => updateBlend(0, v)} />
      <DnaBlend headId={blend[1].headId} value={blend[1].value} onChangeNumber={(v) => updateHeadId(1, v)} onChangeSlider={(v) => updateBlend(1, v)} />
      <DnaBlend headId={blend[2].headId} value={blend[2].value} onChangeNumber={(v) => updateHeadId(2, v)} onChangeSlider={(v) => updateBlend(2, v)} />
      <DnaBlend headId={blend[3].headId} value={blend[3].value} onChangeNumber={(v) => updateHeadId(3, v)} onChangeSlider={(v) => updateBlend(3, v)} />
    </Fieldset>
  );
}
